...
keyb_hypr() {
	...
	echo "$mainMod"
        ...
        mapfile -t BINDINGS < <(grep '^bind[ =]' "$line" | \
            ...
            awk -F',' -v color="$COLOR" -v mod="$mainMod" '{cmd=""; for(i=4;i<=NF;i++) cmd=cmd $i " "; print "<span color=\"" color "\"><b>" mod " + " $2 " " $3 " </b></span>  <i>" cmd "</i>"}')
        all_text+=$(printf "%s\n" "${BINDINGS[@]}")
    done
...
keyb_vim() {
    ...
    for line in "${VIM_KEYB[@]}"; do
	  mapfile -t BINDINGS < <(cat "$line" | \
	    sed "s/^[[:space:]]*//; /^[[:space:]]*$/d; 
		      s/^##[[:space:]]*\(.*\)$/<b>\1<\/b>/;
		         s/^[[:space:]]*\([^-]*\)[[:space:]]-[[:space:]]*\(.*\)$/<span color=\"$COLOR\"><b>\1<\/b><\/span> → <i>\2<\/i>/;")
	  all_text+=$(printf "%s\n" "${BINDINGS[@]}") 
    done
